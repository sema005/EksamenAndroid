...
class ItemAdapter(
    val studentsInfo: ArrayList<ImagesIncoming>,
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        val imagesIncoming: ImagesIncoming = studentsInfo.get(position)

        holder.itemView.setTag(position)
        (((holder.itemView as LinearLayout).getChildAt(0) as LinearLayout).getChildAt(0) as TextView).setText(imagesIncoming.name)
        (((holder.itemView as LinearLayout).getChildAt(0) as LinearLayout).getChildAt(1) as TextView).setText(imagesIncoming.surname)
